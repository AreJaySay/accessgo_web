...
import 'package:pass_slip_management_web/screens/users/components/create_account.dart';
            ...
            floatingActionButton: Padding(
              ...
              child: FloatingActionButton(
                onPressed: () {
                  showDialog(
                    context: context,
                    builder: (_) => CreateAccount(details: {},),
                  );
        ...
        return TableViewRow(
          ...
          cells: record.map((value) {
            return TableViewCell(
              ...
              TextButton(
                onPressed: (){
                  confirmAction.action(context, (){
                    ...
                    FirebaseDatabase.instance.ref().child('users').orderByChild("email").equalTo(value.toString().split(" ")[1]).onChildAdded.forEach((event)async{
                     await ref.update({
              ...
              TextButton(
                onPressed: (){
                  FirebaseDatabase.instance.ref().child('users').orderByChild("email").equalTo(value.toString().split(" ")[1]).onChildAdded.forEach((event){
                    print(event.snapshot.value);
                    showDialog(
                      context: context,
                      builder: (_) => CreateAccount(details: event.snapshot.value as Map,),
                    );
                  });
                },
